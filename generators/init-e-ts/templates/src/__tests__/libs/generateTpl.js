...
export const generateTpl = (appWrapper, pagePath)=>{
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(readFilePath, 'utf8', (error, data) => {
      ...
      originFileInfo = data.replace(/[\t ]*Page\s*\(\s*?{/, `module.exports = {\ngetApp,\nisGoPromiseError,\ntpl_LWPFunc,\n`)
        .replace(/\s*Page\(\s*connectPage\(\s*mapStateToData\s*\)\s*\(\s*\{/, 'module.exports = connectPage(mapStateToData)({\ngetApp,\nisGoPromiseError,\ntpl_LWPFunc,\n')
        .replace(/\s*Page\(\s*Arms.hookmodule.exports\s*=\s*{/, 'module.exports = ({')
        .replace(/import\s*[aA]rms/, '// import Arms')
        .replace(/\s*\)\s*;{0,1}\s*$/, '')
        .replace(/this.setData/g, 'setData').replace(/this.dispatch/g, 'dispatch');
      <% } %>
      ...
      for (let routerKey in routerConfig[testFileName]){
        originFileInfo = originFileInfo.replace(`from '${routerKey}'`, `from '${routerConfig[testFileName][routerKey]}'`).replace(`from "${routerKey}"`, `from "${routerConfig[testFileName][routerKey]}"`);
      }

      originFileInfo = `const tpl_App = ${JSON.stringify(appWrapper.app)};\nlet getApp = ()=> { return tpl_App}; const setData = ()=>{}; const dispatch = ()=>{}; const isGoPromiseError = false; \nconst tpl_LWPFunc = {${tpl_LWPFunc_str}}\n` + originFileInfo;
